...
class Optimizer(object):
    ...
    def tell(self, x, y, fit=True, fixed_dim=None):
        ...
        return self._tell(x, y, fit=fit, fixed_dim=fixed_dim)

    def _tell(self, x, y, fit=True, fixed_dim=None):
        ...
        if (fit and self._n_initial_points <= 0 and
            ...
            with warnings.catch_warnings():
                ...
                est.fit(space.transform(self.Xi), self.yi)
            ...
            samples = np.asarray(space.rvs(n_samples=self.n_points, random_state=self.rng))
            if fixed_dim is not None:
                dim, val = fixed_dim
                samples[:, dim] = val
            X = space.transform(samples)
            ...
            self._next_x = space.inverse_transform(
                next_x.reshape((1, -1)))[0]

        # Pack results
        result = create_result(self.Xi, self.yi, space, self.rng,
                               models=self.models)
